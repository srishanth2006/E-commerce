...
seed_data.py
--------------
Big supermarket seed data: 25 categories, 60+ brands, 8 suppliers, 200+ products
with realistic Indian grocery store prices.
...
for s_data in SUPPLIERS:
    existing = db.query(models.Supplier).filter(models.Supplier.supplier_name == s_data["supplier_name"]).first()
    if existing:
        ...
        print(f"  + Supplier: {s_data['supplier_name']}")
...
db.commit()
print(f"\nDone! Seeded {len(CATEGORIES)} categories, {len(BRANDS)} brands, {len(SUPPLIERS)} suppliers, {bc_counter} products.")
db.close()
